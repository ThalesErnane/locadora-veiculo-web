...
public class AluguelDAO implements Serializable {
	...
	public List<Aluguel> buscarPorDataDeEntregaEModeloCarro(Date dataEntrega, ModeloCarro modelo) {
		...
		if (modelo != null) {
			// select a from Aluguel a where a.dataEntrega between :dataEntregaInicial and
			// :dataEntregaFinal
		...
		if (dataEntrega != null) {
			Calendar dataEntregaInicial = Calendar.getInstance(); // data inicial
			...
			Calendar dataEntregaFinal = Calendar.getInstance(); // data final
